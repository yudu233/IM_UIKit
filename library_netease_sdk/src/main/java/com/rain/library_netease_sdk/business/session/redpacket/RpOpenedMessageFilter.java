...
import com.rain.library_netease_sdk.extension.RedPacketOpenedAttachment;
 ...
 * @filename : RpOpenedMessageFilter
 * @describe : 结合observeReceiveMessage、observeRecentContact 做一个应用层面的消息过滤，过滤掉与自己无关的拆天下乐消息
...
public class RpOpenedMessageFilter {
    ...
    private static Observer<List<IMMessage>> messageObserver = (Observer<List<IMMessage>>) imMessages -> {
        if (imMessages != null) {
    ...
    private static Observer<List<RecentContact>> recentContactObserver = (Observer<List<RecentContact>>) recentContacts -> {
    ...
    private static boolean shouldFilter(IMMessage message) {
        if (message != null && message.getAttachment() instanceof RedPacketOpenedAttachment) {
            ...
            return !attachment.belongTo(NimUIKit.getAccount());
        }
        return false;
    }
