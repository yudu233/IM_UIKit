...
import com.rain.library_netease_sdk.impl.provider.NimUserInfoProvider;

import java.io.IOException;

/**
 * @Author : Rain
 * @CreateDate : 2020/10/9 16:26
 * @Version : 1.0
 * @Descroption : 云信sdk 自定义的SDK选项设置
 */
...
//        configServerAddress(options, context);
    ...
    static String getAppCacheDir(Context context) {
        ...
        if (TextUtils.isEmpty(storageRootPath)) {
            ...
            storageRootPath = Environment.getExternalStorageDirectory() + "/" + NimUIKit.getContext().getPackageName();
        }
    ...
    private static void initStatusBarNotificationConfig(SDKOptions options) {
        ...
        //NeteaseUserPreferences.getStatusConfig()
        StatusBarNotificationConfig userConfig =null;
        if (userConfig == null) {
            userConfig = config;
        } else {
        ...
        // 持久化生效
        NeteaseUserPreferences.setStatusConfig(userConfig);
    ...
    private static StatusBarNotificationConfig loadStatusBarNotificationConfig() {
        ...
        // 点击通知需要跳转到的界面 可以为null，默认为使用包的launcher的入口intent的activity
        //config.notificationEntrance = null;
        //状态栏提醒的小图标的资源ID。如果不提供，使用 app 的 icon
        //config.notificationSmallIconId = 0;
        //消息通知栏颜色
        //config.notificationColor = 0;
        // 通知铃声的uri字符串
        ...
        // save cache，留做切换账号备用
        NimUIKit.setNotificationConfig(config);
        return config;
    }
...
//            return MessageRevokeTip.getRevokeTipContent(item, revokeAccount);
    ...
    private static MixPushConfig buildMixPushConfig() {
        ...
        // 小米推送
        config.xmAppId = "2882303761517902382";
        config.xmAppKey = "5341790257382";
        config.xmCertificateName = "cytxPushMi";

        // 华为推送
        config.hwAppId = "101459213";
        config.hwCertificateName = "cytxPushHua";

        //Vivo推送
        config.vivoCertificateName = "cytxPushVivo";

        //OPPO推送
        config.oppoAppId = "3731349";
        config.oppoAppKey = "7454414273bc4b50878edd7e3f6a6ab9";
        config.oppoAppSercet = "aac61617286b404790d22ea1f373067b";
        config.oppoCertificateName = "cytxPushOPPO";
