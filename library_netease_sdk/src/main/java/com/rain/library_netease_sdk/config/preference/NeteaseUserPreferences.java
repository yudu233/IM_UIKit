package com.rain.library_netease_sdk.config.preference;
 ...
 * @Author : Rain
 * @Version : 1.0
 * @CreateDate :  2020/10/9 17:38
 * @Description : 用户相关本地配置存储
 */
public class NeteaseUserPreferences {
    ...
    private static StatusBarNotificationConfig getConfig(String key) {
        ...
        config.downTimeBegin = jsonObject.get(DOWN_TIME_BEGIN).isJsonNull() ? "" :
                jsonObject.get(DOWN_TIME_BEGIN).getAsString();
        config.downTimeEnd = jsonObject.get(DOWN_TIME_END).getAsString();
        config.downTimeToggle = jsonObject.get(DOWN_TIME_TOGGLE).getAsBoolean();

        Boolean downTimeEnableNotification = jsonObject.get(DOWN_TIME_ENABLE_NOTIFICATION).getAsBoolean();
        config.downTimeEnableNotification = downTimeEnableNotification == null ?
                true : downTimeEnableNotification;
        Boolean ring = jsonObject.get(RING).getAsBoolean();
        config.ring = ring == null ? true : ring;
        Boolean vibrate = jsonObject.get(VIBRATE).getAsBoolean();
        config.vibrate = vibrate == null ? true : vibrate;

        config.notificationSmallIconId = jsonObject.get(NOTIFICATION_SMALL_ICON_ID).getAsInt();
        config.notificationSound = jsonObject.get(NOTIFICATION_SOUND).getAsString();
        config.hideContent = jsonObject.get(HIDE_CONTENT).getAsBoolean();
        config.ledARGB = jsonObject.get(LEDARGB).getAsInt();
        config.ledOnMs = jsonObject.get(LEDONMS).getAsInt();
        config.ledOffMs = jsonObject.get(LEDOFFMS).getAsInt();
        config.titleOnlyShowAppName = jsonObject.get(TITLE_ONLY_SHOW_APP_NAME).getAsBoolean();

        Boolean notificationFolded = jsonObject.get(NOTIFICATION_FOLDED).getAsBoolean();
        config.notificationFolded = notificationFolded == null ? true : notificationFolded;

        Integer notificationFoldType = jsonObject.get(NOTIFICATION_FOLD_TYPE).getAsInt();
        ...
        try {
            config.notificationEntrance = (Class<? extends Activity>) Class.forName(
                    jsonObject.get(NOTIFICATION_ENTRANCE).getAsString());
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
        }
        config.notificationColor = jsonObject.get(NOTIFICATION_COLOR).getAsInt();
    ...
    static SharedPreferences getSharedPreferences() {
        return NeteaseCache.getContext().getSharedPreferences("cytx." + NimUIKit.getAccount(), Context.MODE_PRIVATE);
    }
}
