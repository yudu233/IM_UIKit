...
import com.rain.inputpanel.imageloader.EmoticonImageLoader;
    ...
    protected void bindView(int position, ViewHolder viewHolder) {
        ...
        if (isDelBtn) {
            ...
            if (emoticonEntity != null) {
                try {
                    EmoticonImageLoader.getInstance(viewHolder.iv_emoticon.getContext()).displayImage(emoticonEntity.getIconUri(), viewHolder.iv_emoticon);
                } catch (IOException e) {
                    e.printStackTrace();
                }
                viewHolder.iv_emoticon.setBackgroundResource(R.drawable.bg_emoticon);
